...
- Super Admin bypass: `PermissionsGuard` mein `user.isSuperAdmin` → sab allowed.
...
- Frontend `usePermission` ko user ki permissions chahiye. Abhi:
  - `POST /auth/login` response mein **permissions nahi** hain (sirf `user: { id, fullName, email, roleId, roleName, tenantId, isSuperAdmin }`).
...
- **Fix:** `GET /users/me` add karna (JwtAuthGuard):
  - Response: `{ id, fullName, email, roleId, roleName, isSuperAdmin, permissions: [{ module, action }], assignedBrandIds: [ids] }`
...
- **UI gating (`usePermission(module, action)`):**
  - Sidebar items: sirf dikhaye jab user ke paas module ka `VIEW` permission ho (ya `isSuperAdmin`).
...
- `GET /users/me` ✅ — returns `{ id, fullName, email, roleId, roleName, isSuperAdmin, permissions[], assignedBrandIds[] }` (user.service `me()`).
